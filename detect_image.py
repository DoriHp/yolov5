...
def gen_submission(root_folder):
	...
	# Get image shape
	shapes = dict()
	data_file = csv.reader(open("/home/yolov5/dataset/test.csv"))
	for index, line in tqdm(enumerate(data_file), desc="Retrieve image size"):
		# Skip for the first line 
		if index == 0 or len(line) == 0:
			continue

		file_name = line[0]
		img_width, img_height = line[1:3]
		shapes[file_name] = (int(img_width), int(img_height))
	...
	with open(submission_file, "a+") as f:
		...
		for image_name in tqdm(shapes.keys(), total=len(list(shapes.keys())), desc="Init empty detection for all images"):
			if (image_name + ".txt") not in label_list:
				write_string = image_name + ",14 1 0 0 1 1 "
...
def detect(save_img=False, save_empty_rs=False, save_csv=False):
	...
	for path, img, im0s, vid_cap in dataset:
		...
		for i, det in enumerate(pred):  # detections per image
			...
			# If detected object in this image
			if len(det):
...
if __name__ == '__main__':
	...
	if opt.csv_only:
		gen_submission("runs/detect/exp19")
	else:
		with torch.no_grad():
